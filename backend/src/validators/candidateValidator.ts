...
export const educationSchema = z.object({
  ...
  fieldOfStudy: z.string().max(255, 'Field of study too long').optional(),
...
// Candidate update validation schema (all fields optional)
export const updateCandidateSchema = createCandidateSchema.partial();

// Candidate ID parameter validation
export const candidateIdSchema = z.object({
  id: z.string().regex(/^\d+$/, 'Candidate ID must be a number').transform(Number),
});
